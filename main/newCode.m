...
sourcePos = sofa1.SourcePosition(originRecordIndex,:);  % 声源坐标
micPos    = sofa1.ListenerPosition(originRecordIndex,:);      % 麦克风坐标
maxRef    = 2;             % 只计算 maxRef 次镜像

% 调用函数
T = compute_DOA_ISM_with_plot(roomSize, sourcePos, micPos, maxRef, speedinSec,sofa1); % ImageSource and plot
T.ArrTime

figure(fig_decomposition_sum);
hold on;
for k=1:length(T.ArrTime)
%xline(T.ArrTime(k)*1000, 'g--', ['Nx= ' num2str(T.Nx(k)) ' Ny= ' num2str(T.Ny(k)) ' Nz= ' num2str(T.Nz(k))]);
xline(T.ArrTime(k)*1000, '--', ['Nxyz= ' num2str(T.Nx(k)) ' ' num2str(T.Ny(k)) ' ' num2str(T.Nz(k))]);
