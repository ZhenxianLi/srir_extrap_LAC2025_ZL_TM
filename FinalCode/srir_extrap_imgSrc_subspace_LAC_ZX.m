...
brir_tar=binSound(srir_tar,SH_ambisonic_binaural_decoder);
    ...
    if doBinRenderSONG
        ...
        for i=1:2 % L and R
            binSong_originalIR(:,i)=conv(song_Dry,brir_orig(:,i));
            binSong_generateIR(:,i)=conv(song_Dry,brir_new(:,i));
            binSong_recordIR(:,i)=conv(song_Dry,brir_tar(:,i));
        end
        ...
        Max=max(max(binSong_originalIR));

        audiowrite(['S',num2str(idx_LS_src_orig),'L',num2str(idx_LS_rec_orig),'_Original','.wav'],binSong_originalIR/Max,fs);
        audiowrite(['S',num2str(idx_LS_src_orig),'L',num2str(idx_LS_rec_orig),'_Extrapolation','.wav'],binSong_generateIR/Max,fs);
        audiowrite(['S',num2str(idx_LS_src_orig),'L',num2str(idx_LS_rec_orig),'_Target','.wav'],binSong_recordIR/Max,fs);
        print("audiowrited")
